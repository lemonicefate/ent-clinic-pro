 ...
 * 靜態計算機註冊表
 * 
 * 使用靜態導入的方式管理所有計算機模組，避免 import.meta.glob 的問題。
 */

import { CalculatorModule, SearchQuery } from '../types';
import { StaticModuleLoader } from './StaticModuleLoader';
import { ErrorHandler } from './ErrorHandler';

export class StaticCalculatorRegistry {
  private static instance: StaticCalculatorRegistry | null = null;
  ...
  private loader: StaticModuleLoader;
  private errorHandler: ErrorHandler;

  private constructor() {
    this.loader = new StaticModuleLoader();
   ...
   * 獲取註冊表單例
   */
  static getInstance(): StaticCalculatorRegistry {
    if (!this.instance) {
      this.instance = new StaticCalculatorRegistry();
    }
  ...
  async initialize(): Promise<void> {
    ...
    console.log('🚀 Initializing Static Calculator Registry...');
    ...
    try {
      ...
      console.log(`✅ Static Calculator Registry initialized with ${this.modules.size} modules`);
      console.log(`📊 Categories: ${Array.from(this.categories.keys()).join(', ')}`);

    } catch (error) {
      console.error('❌ Failed to initialize Static Calculator Registry:', error);
  ...
  async reload(): Promise<void> {
    console.log('🔄 Reloading Static Calculator Registry...');
