...
export class ModuleLoaderFixed {
  ...
  async loadAllModules(): Promise<ModuleLoadResult[]> {
    ...
    try {
      ...
      console.log('Module paths:', Object.keys(moduleImports));
  ...
  private async loadSingleModule(
    ...
    try {
      ...
      if (!moduleExport) {
        return {
          success: false,
          error: `Module at ${path} returned null or undefined`
        };
      }

      const module = moduleExport as CalculatorModule;
